...
export const useTournamentTripStore = defineStore('tournamentTrip', () => {
  // --- STATE ---
  ...
  function loadTripsFromLocalStorage() {
    try {
      const stored = localStorage.getItem('tournament-trips');
      if (stored) {
        savedTrips.value = JSON.parse(stored);
      }
    } catch (error) {
      console.error("Error loading tournament trips from localStorage:", error);
      savedTrips.value = [];
    }
  }
  ...
  function saveTripsToLocalStorage() {
    try {
      localStorage.setItem('tournament-trips', JSON.stringify(savedTrips.value));
    } catch (error) {
      console.error("Error saving tournament trips to localStorage:", error);
    }
  }
  ...
  function saveCurrentTrip() {
    ...
    const existingIndex = savedTrips.value.findIndex(t => t.id === tripData.id);
    if (existingIndex !== -1) {
      savedTrips.value[existingIndex] = tripData;
    } else {
      savedTrips.value.push(tripData);
      currentTripId.value = tripData.id;
    }

    saveTripsToLocalStorage();
    localStorage.setItem('tournament-trip-state', JSON.stringify(tripData));
  }

  function loadTrip(tripId) {
    const tripToLoad = savedTrips.value.find(t => t.id === tripId);
    if (!tripToLoad) {
      console.error("Tournament trip not found with ID:", tripId);
      return;
    }
    ...
    localStorage.setItem('tournament-trip-state', JSON.stringify(tripToLoad));
  }
  ...
  function resetCurrentTrip() {
    ...
    dailyResults.value = {};
    setPlayerCount(4);
    localStorage.removeItem('tournament-trip-state');
  }
